/*
 * Super-Rogue in the browser: draws the panes of the curses shim (port/be_web.c
 * calls Module.xr), keyboard input, tiling windows, sound, saves in
 * IndexedDB. Loaded before srogue-core.js.
 */
(function () {
	'use strict';
	...
	var DIR = '/srogue/save';                         /* IDBFS mount: saves, scores, layout */
	var SAVE = DIR + '/srogue.sav', LAYOUT_FILE = DIR + '/web-layout.json';
	...
	window.Module = {
		...
		preRun: [function () {
			...
			FS.chdir('/srogue');
			Module.ENV.HOME = DIR;               /* srogue.sav (md_gethomedir) */
			Module.ENV.ROGUEHOME = DIR;          /* score file */
			Module.ENV.USER = 'rogue';
			var who = '';                        /* whoami: getpwuid() is web_user, so ask once */
			try { who = localStorage.getItem('srogue-name') || ''; } catch (err) { /* no storage */ }
			if (!who) { who = (prompt('What is your name, adventurer?', '') || '').replace(/[,\n]/g, '').trim().slice(0, 30); try { if (who) localStorage.setItem('srogue-name', who); } catch (err) { /* no storage */ } }
	...
	function crashed(err) {
		...
		console.error('[srogue] crash:', err);
	...
	window.addEventListener('error', function (e) {
		...
		if (e.error instanceof WebAssembly.RuntimeError || /srogue-core/.test(e.filename || '')) crashed(e.error || e.message);
	});
